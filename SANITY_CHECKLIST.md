...
Smoke-level provisional marks for sections 4-9 are supported by `SMOKE_MANUAL_4_9.md` (manual artifact check + code-sanity review).
...
- [x] Phase objectives are documented consistently in code comments and README:
  - [x] E0 reproduction
...
- [x] Core identity fields present:
  - [x] `algorithm`
  - [x] `seed`
  - [x] `phase`
  - [x] `train_envs`
...
- [x] Failed runs are explicitly distinguishable from valid low-performance runs (status/error marker).
...
- [x] No silent missing values in grouping columns used by analysis scripts.
...
- [x] One E0 summary table is generated:
  - [x] average accuracy
  - [x] worst-domain accuracy
  - [x] regret (exact or clearly labeled approximate)
...
- [x] No duplicate-run inflation in final tables.
...
- [x] Grouped summaries do not mix phases, seeds, or condition definitions.
...
- [x] Final pass: verify README/methods text documents exact reduced vs full sweep differences.

## Minimum Deliverables (If Time-Constrained)

- [x] E0 reduced reproduction plot + table.
