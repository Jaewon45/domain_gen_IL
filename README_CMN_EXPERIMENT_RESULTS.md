...
| E3b tail-support analysis | Four-condition design complete: 60 records for 3 seeds, 4 conditions, and 5 algorithms | `results/E3b_tail_support/` |
...
- `identification_width_by_alpha.csv`
- `identification_width_by_alpha.png`
...
The identification-width diagnostic is generated by `CMNIST/analyze_identification_width.py`. For each alpha, sample size, long-tail exponent, and missing-tail fraction, it reports an analytical bounded-risk width, simulated lower/upper CVaR under missing-domain completions, and empirical interval widths from repeated source samples. The analytical diagnostic assumes missing deployment mass epsilon is known and missing-domain risks lie in `[0,1]`, using the conservative bound

$$
\Delta_{\mathrm{ID}}(\alpha;\varepsilon)
= \min\left(1,\frac{\varepsilon}{1-\alpha}\right).
$$
